...
from blink_capture import TripleBlinkCapture, BlinkCaptureConfig
...
# robot spawn and board pose from your sim
...
# ROS publisher
# -----------------------------
class GazeTargetPublisher(Node):
    def __init__(self):
        super().__init__("gaze_target_publisher")
        self.pub = self.create_publisher(PointStamped, "/gaze_target_point", 10)

    def publish_point(self, xyz: np.ndarray, frame_id: str = "base_link"):
        msg = PointStamped()
        ...
        self.get_logger().info(
            f"Published target in {frame_id}: "
...
def main():
    ...
    blink_capture = TripleBlinkCapture(
        BlinkCaptureConfig(
            blink_gap_s=0.65,
            sequence_timeout_s=2.0,
    ...
    print("\nRunning live 3D fixation + triple-blink capture...")
    print("Triple blink to publish the current target to /gaze_target_point\n")
    ...
    try:
        with PupilStream(pupil_cfg) as pupil, RealSenseStream(rs_cfg) as rs_cam:
            while True:
                ...
                p0_conf = None if last_pupil0 is None else last_pupil0.get("confidence", None)
                p1_conf = None if last_pupil1 is None else last_pupil1.get("confidence", None)

                if target_robot is not None:
                    if blink_capture.update(p0_conf, p1_conf, now=time.time()):
                        ros_node.publish_point(target_robot, frame_id="base_link")
                        print("TRIPLE BLINK DETECTED -> target published")
                ...
                if SHOW_WINDOWS:
                    ...
                    cv2.putText(rs_vis, "Triple blink to publish target",
